...
model_check_point = "amazon_recognition.h5"
...
# load data
raw_data = pd.read_csv('human_keypoint.csv', header=0)
...
model.fit(X_train, Y_train, batch_size=32, epochs=150, verbose=1, validation_data=(X_test, Y_test), callbacks=[his])
model.summary()
his.loss_plot('epoch')
model.save(model_check_point)
...
plt.figure()
class_names =["handy", "moving", "product_search"]  #['squat', 'stand', 'walk', 'wave']
